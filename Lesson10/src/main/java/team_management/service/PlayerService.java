...
public class PlayerService {
    ...
    //Khởi tạo constructor để chứa danh sách bên dưới (có 3 cách khởi tạo: trực tiếp, static, tạo constructor)
    ...
    private ArrayList<Player> getPlayers(Position position, int number) {
        ...
        while (count < number) {
            //random ra một player ngẫu nhiên
            int rdIndex = rd.nextInt(players.size());

            //Nếu player được random có cùng vị trí và chưa ở trong ds thì thêm vào ds
            Player rdPlayer = players.get(rdIndex);
            if (rdPlayer.getPositon().equals(position) && !selectedPlayers.contains(rdPlayer)) {
